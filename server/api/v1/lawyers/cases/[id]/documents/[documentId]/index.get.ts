export default defineEventHandler(async (event) => {
  ...
  try {
    const response = await $fetch(`${apiBase}/lawyers/cases/${caseId}/documents/${documentId}`, {
      method: 'GET',
      headers: {
    ...
    return {
      status: 200,
      message: (responseData.message as string) || 'Document retrieved successfully',
      data: response
    }
  } catch (error) {
    let statusCode = 401
    ...
    if (error && typeof error === 'object') {
      ...
      message = (data?.message as string) || 'Failed to retrieve document'
