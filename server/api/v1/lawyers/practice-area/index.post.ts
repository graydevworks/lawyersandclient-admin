export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const response = await $fetch(`${apiBase}/lawyers/practice-areas`, {
      method: 'POST',
      headers: {
    ...
    return {
      status: 200,
      message: (responseData.message as string) || 'Practice areas saved successfully',
      data: response
    }
  } catch (error) {
    let statusCode = 401
    let message = 'Failed to save practice areas'

    if (error && typeof error === 'object') {
      const err = error as Record<string, unknown>
      statusCode = (err.statusCode as number) || (err.status as number) || 401
      const data = err.data as Record<string, unknown> | undefined
      message = (data?.message as string) || 'Failed to save practice areas'
