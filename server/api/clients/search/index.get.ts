export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const response = await $fetch(`${apiBase}/admin/clients/search`, {
      method: 'GET',
      query,
      headers: {
    ...
    return {
      status: 200,
      message: (responseData.message as string) || 'Client search results fetched successfully',
      data: response
    }
  } catch (error) {
    let statusCode = 401
    ...
    if (error && typeof error === 'object') {
      ...
      message = (data?.message as string) || 'Failed to search clients'
