export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const response = await $fetch(`${apiBase}/admin/cases`, {
      method: 'GET',
      query,
      headers: {
    ...
    return {
      status: 200,
      message: (responseData.message as string) || 'Cases fetched successfully',
      data: response
    }
  } catch (error) {
    throwApiError(error, 'Failed to fetch cases')
  }
})
