export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    return {
      status: 200,
      message: (responseData.message as string) || 'Case reinstated successfully',
      data: response
    }
  } catch (error) {
    let statusCode = 401
    let message = 'Failed to reinstate case'

    if (error && typeof error === 'object') {
      const err = error as Record<string, unknown>
      statusCode = (err.statusCode as number) || (err.status as number) || 401
      const data = err.data as Record<string, unknown> | undefined
      message = (data?.message as string) || 'Failed to reinstate case'
