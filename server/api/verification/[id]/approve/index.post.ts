export default defineEventHandler(async (event) => {
  ...
  try {
    const response = await $fetch(`${apiBase}/admin/verification-queue/${event.context.params?.id}/approve`, {
      method: 'POST',
      headers: {
    ...
    return {
      status: 200,
      message: (responseData.message as string) || 'Verification approved successfully',
      data: response
    }
  } catch (error) {
    let statusCode = 401
    ...
    if (error && typeof error === 'object') {
      ...
      message = (data?.message as string) || 'Failed to approve verification'
