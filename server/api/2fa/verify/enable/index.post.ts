export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const response = await $fetch(`${apiBase}/admin/auth/2fa/enable`, {
      method: 'POST',
      headers: {
    ...
    return {
      status: 200,
      message: (responseData.message as string) || '2FA enabled successfully',
      data: response
    }
  } catch (error) {
    let statusCode = 401
    ...
    if (error && typeof error === 'object') {
      ...
      message = (data?.message as string) || 'Failed to enable 2FA'
