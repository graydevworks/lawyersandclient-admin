export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const response = await $fetch(`${apiBase}/admin/settings/app-banners/reorder`, {
      method: 'POST',
      headers: {
    ...
    return {
      status: 200,
      message: (responseData.message as string) || 'Banners reordered successfully',
      data: response
    }
  } catch (error) {
    let statusCode = 401
    ...
    if (error && typeof error === 'object') {
      ...
      message = (data?.message as string) || 'Failed to reorder banners'
