export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const lawyerId = event.context.params?.id

    // First, fetch the lawyer details to get the document URL
    const lawyerResponse = await $fetch(`${apiBase}/admin/lawyers/${lawyerId}`, {
      method: 'GET',
      headers: {
    ...
    const lawyerData = lawyerResponse as { data?: { documents?: Array<{ url?: string, file_url?: string, document_url?: string }> } }
    const documents = lawyerData?.data?.documents || []
