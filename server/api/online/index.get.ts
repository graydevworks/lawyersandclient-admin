export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const response = await $fetch(`${apiBase}/admin/online-users`, {
      method: 'GET',
      query,
      headers: {
    ...
    return {
      status: 200,
      message: (responseData.message as string) || 'Online users fetched successfully',
      data: response
    }
  } catch (error) {
    throwApiError(error, 'Failed to fetch online users')
  }
})
